...
![RoboShop Architecture](docs/images/roboshop-architecture.jpg)

The frontend (Nginx) is the only publicly reachable service, accessed via `daws88s.online`. It reverse-proxies to five internal application services (catalogue, user, cart, shipping, payment), each of which connects to the specific datastore(s) it needs — MongoDB, Redis, MySQL, or RabbitMQ. Dispatch (not shown) connects only to RabbitMQ as a consumer and has no inbound calls.
